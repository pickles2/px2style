window.broccoliModulePx2StyleImageList = function(broccoli){
	...
	const languageCsv = require('./modules/language.csv');
	...
	// 	this.lb.setLang( 'ja' );
	// 	console.log('=-=-=-=-= languageCsv', languageCsv);
	// 	// console.log('=-=-=-=-= lb', broccoli.lb);
	// 	console.log('=-=-=-=-= lb', this.lb.get('ui_label.select'));
	// 	// console.log('=-=-=-=-= lang', broccoli.lb.getLang());
	// });

	/**
	 * プレビュー用の簡易なHTMLを生成する
	 */
	this.mkPreviewHtml = mkPreviewHtml.mkPreviewHtml;

	/**
	 * エディタUIを生成
	 */
	this.mkEditor = mkEditor.mkEditor;
